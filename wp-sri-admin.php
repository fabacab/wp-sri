...
class WP_SRI_Known_Hashes_List_Table extends WP_List_Table {
    ...
    public function __construct () {
        // Get our excluded option array.
        $this->sri_exclude = get_option( WP_SRI_Plugin::prefix . 'excluded_hashes', array() );
    ...
    public function get_columns () {
        return array(
            ...
            //'fallback' => esc_html__( 'Fallback', 'wp-sri' ),
        );
    }
    ...
    public function prepare_items () {
        ...
        //$fallbacks    = get_option( WP_SRI_Plugin::prefix . 'fallback_by_hashes', array() );
        if ( ! empty( $_POST['s'] ) ) {
        ...
        foreach ( $known_hashes as $url => $hash ) {
            ...
            //$fallback = ( empty( $fallbacks[ $hash ] ) ) ? '' : $fallbacks[ $hash ];
            $items[] = array(
                'url' => $url,
                'hash' => $hash,
                'exclude' => $exclude,
    ...
    protected function column_exclude( $item ) {
        ...
        // image shown during AJAX request to indicate something is happening.
        $loading = plugin_dir_url( __FILE__ ) . 'css/working.gif';
        return sprintf(
            '<input disabled="disabled" type="checkbox" class="sri-exclude" id="%s" %s><span class="sri-loading"><img src="%s" /> </span>',
            $url, $checked, $loading
        );
    }

    /**
     * Create our output for the URL column.
     *
     * @param array $item
     *
     * @return string
    ...
    /**
     * Create our output for the Fallback column.
     *
     * @param array $item
     *
     * @return string
...
//        return sprintf(
//            '<input name="fallback[][' . esc_attr( rawurlencode( $item['url'] ) ) . ']" class="code" value="%1$s" />',
//            $item[ 'fallback' ]
//        );
//    }
